...
namespace CS2013
{
    class ClCourtierCapsuleSante : ClCourtier
    {
        ...
        private String NomTable
        {
            get
            {
                return "capsule_sante";
            }
        }

        public ClCourtierCapsuleSante()
        { }

        private static ClCourtierCapsuleSante m_instance = null;

        public static ClCourtierCapsuleSante GetInstance()
        {
            if (m_instance == null)
                m_instance = new ClCourtierCapsuleSante();

            return m_instance;
        }

        public bool Insérer(ClCapsuleSante p_capsule)
        {
        ...
        public bool Modifier(ClCapsuleSante p_capsule)
        {
            using (SqlConnection cn = new SqlConnection(StringDeConnexion))
            {
                SqlCommand cmd = new SqlCommand(
        ...
        public List<ClCapsuleSante> ObtenirTous()
        {
            using (SqlConnection cn = new SqlConnection(StringDeConnexion))
            {
                SqlCommand cmd = new SqlCommand(
                ...
                List<ClCapsuleSante> capsules = new List<ClCapsuleSante>();

                while (reader.Read())
                {
                    capsules.Add(new ClCapsuleSante(reader.GetString(0), reader.GetString(1), reader.GetDateTime(2), reader.GetString(3)));
                }
        ...
        public ClCapsuleSante ObtenirPlusRecente()
        {
            using (SqlConnection cn = new SqlConnection(StringDeConnexion))
            {
                SqlCommand cmd = new SqlCommand(
                ...
                ClCapsuleSante capsule = new ClCapsuleSante();
